...
TEST_CASE("SkipList:LayersTest:Expect16", "[Sample][SkipList][Layers]") {
    ...
    skipList.find(0) = 10;
    REQUIRE(skipList.find(0) == 10);
}

TEST_CASE("SkipList:InsertTestStrings:ExpectFoundValueEqualsInsertedValueAndChange",
          "[Required][SkipList]") {
    proj2::SkipList<std::string, std::string> skipList;

    skipList.insert("Shindler", "ICS 46");
    skipList.find("Shindler") = "ICS 45C";
    REQUIRE(skipList.find("Shindler") == "ICS 45C");
    REQUIRE(skipList.layers() == 4);
    skipList.printSkipList();
}
...
TEST_CASE("FAILCASES")
{
    ...
    skipList.insert("TA", "OFFICEHOURS");
    REQUIRE(skipList.isSmallestKey("TA") == true);
    REQUIRE(skipList.isLargestKey("TA") == true);

    skipList.insert("BA", "SCHOOL");
    skipList.printSkipList();

    REQUIRE(skipList.nextKey("BA") =="TA");
    REQUIRE_THROWS(skipList.nextKey("TA"));
    REQUIRE_THROWS(skipList.previousKey("BA"));


    skipList.erase("TA");
    skipList.erase("BA");
    REQUIRE_THROWS(skipList.isLargestKey("BA"));
